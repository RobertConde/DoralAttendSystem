...
class MySource_Sniffs_Channels_IncludeSystemSniff extends PHP_CodeSniffer_Standards_AbstractScopeSniff
{
    ...
    protected function processTokenWithinScope(
        ...
        if (preg_match('|/systems/(.*)/([^/]+)?actions.inc$|', $fileName, $matches) !== 0) {
            ...
            // have to require the system in which it exists.
        ...
        // Now go searching for includeSystem, includeAsset or require/require
        ...
        if (in_array(strtolower($className), $includedClasses) === false) {
            $error = 'Static method called on non-included class or system "%s"; require system with Channels::includeSystem() or require class with require_once';
    ...
    protected function processTokenOutsideScope(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        // Go searching for includeSystem, includeAsset or require/require
        ...
        if (in_array(strtolower($className), $includedClasses) === false) {
            if ($tokens[$stackPtr]['code'] === T_EXTENDS) {
                $error = 'Class extends non-included class or system "%s"; require system with Channels::includeSystem() or require class with require_once';
                $data  = array($className);
